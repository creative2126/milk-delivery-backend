...
async function testSubscriptionCreation() {
    ...
    try {
        ...
        if (createResponse.status === 201) {
            ...
            const hasActiveSubscription = afterResponse.data.hasActiveSubscription &&
                afterResponse.data.subscription?.subscription_type === testUser.subscription_type;

            if (hasActiveSubscription) {
                console.log('✅ SUCCESS: New subscription is immediately visible');
                console.log('New subscription ID:', afterResponse.data.subscription.id);
            } else {
                console.log('❌ FAILURE: New subscription not found in response');
                console.log('Expected subscription_type:', testUser.subscription_type);
                console.log('Actual subscription_type:', afterResponse.data.subscription?.subscription_type);
            }
        } else {
